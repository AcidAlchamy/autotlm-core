/*
 * DrivonFrame.h — the canonical Drivon telemetry frame.
 *
 ...
 * the JSON serialization match the Drivon Dash / ingest contract exactly
 ...
 */
#ifndef DRIVON_FRAME_H
#define DRIVON_FRAME_H

#include <Arduino.h>

/** Maximum characters of a VIN (17) plus terminator, rounded up. */
#define DRIVON_VIN_LEN 24
/** Comma-separated DTC list buffer, e.g. "P0171,P0420". */
#define DRIVON_DTC_STR_LEN 64
/** Device id / type string buffers. */
#define DRIVON_ID_LEN 20
#define DRIVON_TYPE_LEN 12
...
struct DrivonFrame {
  // ---- device ----
  char deviceId[DRIVON_ID_LEN];     ///< unit id (defaults to the chip id)
  char deviceType[DRIVON_TYPE_LEN]; ///< board/device type string
  ...
  char  vin[DRIVON_VIN_LEN];        ///< vehicle identification number
  ...
  char dtc[DRIVON_DTC_STR_LEN];     ///< comma-separated codes, e.g. "P0171,P0420"
  ...
  /**
   * Serialize to the Drivon ingest JSON (same shape the dashboards already
