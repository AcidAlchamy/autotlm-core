...
size_t DrivonFrame::toJson(char* buf, size_t cap) const {
  ...
  jcat(buf, cap, len,
       "{\"source\":\"device\","
       "\"device\":{\"id\":\"%s\",\"type\":\"%s\",\"mems\":\"%s\",\"fw_gnss\":\"%s\",\"rssi\":%d},",
       deviceId, deviceType, mems, gnssUp ? "OK" : "NO", rssi);

  jcat(buf, cap, len,
       "\"obd\":{\"connected\":%s,\"speed_kph\":%d,\"rpm\":%d,\"coolant_c\":%d,"
       "\"load_pct\":%d,\"throttle_pct\":%d,\"volts\":%.1f,\"vin\":\"%s\",\"pids\":{",
       obdConnected ? "true" : "false", speedKph, rpm, coolantC, loadPct,
       throttlePct, volts, vin);
