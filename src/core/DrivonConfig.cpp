/*
 * DrivonConfig.cpp — NVS-backed settings + persistent diagnostics.
 * Part of Drivon Core — MIT licensed.
 */
#include "DrivonConfig.h"

#if defined(ESP32)

bool DrivonConfig::begin() {
  if (m_up) return true;
  if (!m_diag.begin("drivon-diag", false)) return false;
...
void DrivonConfig::saveWifi(const char* ssid, const char* pass) {
  m_prefs.begin("drivon-wifi", false);
  m_prefs.putString("ssid", ssid ? ssid : "");
  m_prefs.putString("pass", pass ? pass : "");
  m_prefs.end();
}

bool DrivonConfig::loadWifi(char* ssid, size_t ssidCap, char* pass, size_t passCap) {
  m_prefs.begin("drivon-wifi", true);
...
void DrivonConfig::saveDiag(const DrivonDiag& d) {
...
void DrivonConfig::printPrevSession(Stream& out) const {
  out.printf(
...
void DrivonConfig::putString(const char* key, const char* value) {
  m_prefs.begin("drivon-user", false);
  m_prefs.putString(key, value ? value : "");
  m_prefs.end();
}

size_t DrivonConfig::getString(const char* key, char* out, size_t cap, const char* fallback) {
  m_prefs.begin("drivon-user", true);
...
void DrivonConfig::putInt(const char* key, int32_t value) {
  m_prefs.begin("drivon-user", false);
  m_prefs.putInt(key, value);
  m_prefs.end();
}

int32_t DrivonConfig::getInt(const char* key, int32_t fallback) {
  m_prefs.begin("drivon-user", true);
...
bool DrivonConfig::begin() { m_up = true; return true; }
void DrivonConfig::saveWifi(const char*, const char*) {}
bool DrivonConfig::loadWifi(char* ssid, size_t ssidCap, char* pass, size_t passCap) {
  if (ssid && ssidCap) ssid[0] = 0;
  if (pass && passCap) pass[0] = 0;
  return false;
}
void DrivonConfig::saveDiag(const DrivonDiag&) {}
void DrivonConfig::printPrevSession(Stream& out) const { out.println("DIAG: not available on this platform"); }
void DrivonConfig::putString(const char*, const char*) {}
size_t DrivonConfig::getString(const char*, char* out, size_t cap, const char* fallback) {
  if (!out || !cap) return 0;
  strncpy(out, fallback ? fallback : "", cap - 1);
  out[cap - 1] = 0;
  return strlen(out);
}
void DrivonConfig::putInt(const char*, int32_t) {}
int32_t DrivonConfig::getInt(const char*, int32_t fallback) { return fallback; }
